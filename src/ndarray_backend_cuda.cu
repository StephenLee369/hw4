...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(int i = shape.size - 1; i >= 0; i--){
      x += strides.data[i] * (y % shape.data[i]);
      y = y / shape.data[i];
    }
    out[gid] = a[x]; 
  }
...
//__global__ void Matmul3Kernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P) {
//  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
//  if (gid < M){
//    for(int j = 0; j < P; j++){
//      out[gid * P + j] = 0;
//      for(int k = 0; k < N; k++){
//          out[gid * P + j] += a[N * gid + k] * b[N * j + k];
//      }
//    }
//    
//  }
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  MatmulKernel<<<(M + 511) / 512, 512>>>(a.ptr, b.ptr, out->ptr, M, N, P);
...
void Matmul2(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  Matmul2Kernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  cudaDeviceSynchronize();
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size){
    int index = reduce_size * gid;
    out[gid] = a[index];
    for(int j = index; j < reduce_size + index; j++){
      if(a[j] > out[gid]){
        out[gid] = a[j];
      }
    }
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size){
  ...
  if(gid < size){
    int index = reduce_size * gid;
    out[gid] = a[index];
    for(int j = index + 1; j < reduce_size + index; j++){
      {
        out[gid] += a[j];
      }
    }
...
PYBIND11_MODULE(ndarray_backend_cuda, m) {
  ...
  m.def("matmul", Matmul);
  //m.def("matmul2", Matmul2);
  //m.def("matmul3", Matmul3);
