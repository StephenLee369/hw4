...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        for i in range(len(new_axes)):
            new_shape.append(self.shape[new_axes[i]])
            new_stride.append(self.strides[new_axes[i]])
        new_shape = tuple(new_shape)
        new_stride = tuple(new_stride)
        #new_strides = self.compact_strides(new_shape)
        return NDArray.make(
            new_shape, strides=new_stride, device=self.device, handle=self._handle
        )
        ### BEGIN YOUR SOLUTION
        raise NotImplementedError()
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        # Process each index in the tuple, converting everything to slice objects
        idxs = tuple(
            [
        ...
        # Ensure the number of slices matches the number of dimensions in the array
        assert len(idxs) == self.ndim, "Need indexes equal to number of dimensions"

        # Calculate the new shape, stride, and offset for the sub-array
        new_shape = []
        new_strides = []
        offset = self._offset
    ...
    def matmul3(self, other):
        ...
        m, n, p = self.shape[0], self.shape[1], other.shape[0]
